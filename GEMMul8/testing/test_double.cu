...
void accuracy_check(std::string &deviceName, std::string &dateTime) {
    ...
    for (auto &phi : phi_list) {
        for (auto &k : k_list) {
            ...
            double beta  = 0.0;
            gpuDeviceSynchronize();
            gpublasGemmEx(handle, GPUBLAS_OP_N, GPUBLAS_OP_N, m, n, k, &alpha, devA, GPU_R_64F, m, devB, GPU_R_64F, k, &beta, devC, GPU_R_64F, m, GPUBLAS_COMPUTE_64F, GPUBLAS_GEMM_DEFAULT);
            gpuDeviceSynchronize();
            ...
            for (auto &num_moduli : num_moduli_list) {
                ...
                gpuDeviceSynchronize();
                timestmp = gemmul8::gemm<double>(handle, GPUBLAS_OP_N, GPUBLAS_OP_N, m, n, k, &alpha, devA, m, devB, k, &beta, devC, m, num_moduli, true, work_gemm);
                gpuDeviceSynchronize();
                gpuMemcpy(cpuC, devC, m * n * sizeof(double), gpuMemcpyDeviceToHost);
                gpuDeviceSynchronize();
                eval::err::gemm_err(m, n, cpuC, cpuC1, cpuC2, errmax, errmed);
            ...
            for (auto &num_moduli : num_moduli_list) {
                ...
                gpuDeviceSynchronize();
                timestmp = gemmul8::gemm<double>(handle, GPUBLAS_OP_N, GPUBLAS_OP_N, m, n, k, &alpha, devA, m, devB, k, &beta, devC, m, num_moduli, false, work_gemm);
                gpuDeviceSynchronize();
                gpuMemcpy(cpuC, devC, m * n * sizeof(double), gpuMemcpyDeviceToHost);
                gpuDeviceSynchronize();
                eval::err::gemm_err(m, n, cpuC, cpuC1, cpuC2, errmax, errmed);
...
void watt_check(std::string &deviceName, std::string &dateTime) {
    ...
    for (auto &n : n_list) {
        ...
        for (auto &num_moduli : num_moduli_list) {
            ...
            res = getWatt::getWatt(
                [&]() {
                    gemmul8::gemm<double>(handle, GPUBLAS_OP_N, GPUBLAS_OP_N, m, n, k, &alpha, devA, m, devB, k, &beta, devC, m, num_moduli, true, work_gemm);
                },
                m,
                n,
                k);
        ...
        for (auto &num_moduli : num_moduli_list) {
            ...
            res = getWatt::getWatt(
                [&]() {
                    gemmul8::gemm<double>(handle, GPUBLAS_OP_N, GPUBLAS_OP_N, m, n, k, &alpha, devA, m, devB, k, &beta, devC, m, num_moduli, false, work_gemm);
                },
                m,
                n,
                k);
