...
void accuracy_check(std::string &deviceName, std::string &dateTime) {
    std::string fileName = "oz2_results_fC_accuracy_" + deviceName + "_" + dateTime + ".csv";
    ...
    for (auto &phi : phi_list) {
        for (auto &k : k_list) {
            ...
            // C := A*B by FP32
            //--------------------
            gpuFloatComplex alphaf = make_gpuFloatComplex(1.0f, 0.0f);
            gpuFloatComplex betaf  = make_gpuFloatComplex(0.0f, 0.0f);
            gpuDeviceSynchronize();
            ...
            //--------------------
            gpuDeviceSynchronize();
            gpublasGemmEx(handle, GPUBLAS_OP_N, GPUBLAS_OP_N, m, n, k, &alphaf, devAf, GPU_C_32F, m, devBf, GPU_C_32F, k, &betaf, devCf, GPU_C_32F, m, GPUBLAS_COMPUTE_32F_FAST_TF32, GPUBLAS_GEMM_DEFAULT);
            gpuDeviceSynchronize();
            gpuMemcpy(cpuCf, devCf, m * n * sizeof(gpuFloatComplex), gpuMemcpyDeviceToHost);
            gpuDeviceSynchronize();
            eval::err::gemm_err_C(m, n, cpuCf, cpuCd, errmax, errmed);
...
void time_check(std::string &deviceName, std::string &dateTime) {
    std::string fileName = "oz2_results_fC_time_" + deviceName + "_" + dateTime + ".csv";
    std::ofstream outFile(fileName);
    outFile << "phi,m,n,k,"
            << "function,"
    ...
    for (auto &n : n_list) {
        ...
        gpuDoubleComplex *cpuCd      = workd_cpu;
        gpuFloatComplex *cpuCf       = workf_cpu;
        gpuFloatComplex *devAf       = reinterpret_cast<gpuFloatComplex *>(work_gpu);
        gpuFloatComplex *devBf       = devAf + m * k;
        gpuFloatComplex *devCf       = devBf + k * n;
        ...
        //--------------------
        void *workd_gpu;
        gpuMalloc(&workd_gpu, (m * k + k * n + m * n) * sizeof(gpuDoubleComplex));
        gpuDoubleComplex *devAd = reinterpret_cast<gpuDoubleComplex *>(workd_gpu);
        gpuDoubleComplex *devBd = devAd + m * k;
        gpuDoubleComplex *devCd = devBd + k * n;
        makemat::f2d_C(m, k, devAf, devAd);
        makemat::f2d_C(k, n, devBf, devBd);
        ...
        gpuMemcpy(cpuCf, devCf, m * n * sizeof(gpuFloatComplex), gpuMemcpyDeviceToHost);
        gpuDeviceSynchronize();
        eval::err::gemm_err_C(m, n, cpuCf, cpuCd, maxerr, mederr);
...
void watt_check(std::string &deviceName, std::string &dateTime) {
    std::string fileName = "oz2_results_fC_watt_" + deviceName + "_" + dateTime + ".csv";
    std::ofstream outFile(fileName);
    outFile << "phi,m,n,k,"
            << "function,"
            << "relerr_max,relerr_med,"
